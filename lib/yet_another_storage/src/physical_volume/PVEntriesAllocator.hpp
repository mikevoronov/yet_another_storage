...
class PVEntriesAllocator {
 public:
  ...
  OffsetType ExpandPV(PVDeviceDataReaderWriterType &data_reader_writer_, OffsetType free_entry_offset) {
    ...
    for (int64_t written_clusters = 0; written_clusters < allocate_clusters_count; 
      ...
      for (int32_t new_cluster_id = 0; new_cluster_id < simultaneously_allocated_clusters_; ++new_cluster_id) {
        ...
        next_free_entry_offset += cluster_size_;
      }
 ...
 private:
  ...
  const double extend_factor_ = 1.5;
};
} // namespace pv
} // namespace yas
