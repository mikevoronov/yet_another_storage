...
class PVEntriesManager {
 ...
 public:
  ...
  void SaveStartEntries(const OffsetType index_offset) {
    PVHeader pv_header;
    ...
    data_reader_writer_.Write<FreelistHeaderType>(sizeof pv_header, freelist_helper_.GetBins());
  ...
  storage_value_type GetEntryContent(OffsetType offset) {
    ...
    return std::visit([this, offset](auto &&value) {
        auto &&storage_result = getEntryContent<std::decay_t<decltype(value)>::HeaderType>(offset);
 ...
 private:
  ...
  EntryType getEntryContent(OffsetType offset) {
    ...
    if constexpr(std::is_same_v<HeaderType, ComplexTypeHeader>) {
      ...
      return EntriesTypeConverter::ConvertToEntryType(header.value_type_, header.value_);
    }
  }
  ...
  bool recoverAndPushNextEntry(OffsetType offset) {
    ...
    freelist_helper_.PushFreeEntry(next_free_entry, sizeof header);
    return true;
  }
