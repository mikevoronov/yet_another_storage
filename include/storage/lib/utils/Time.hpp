...
class Time {
 public:
  constexpr explicit Time(uint32_t expired_time_low = 0, uint16_t expired_time_high = 0)
  ...
  explicit Time(time_t expired_time) {
    ...
    if (time & 0xFFFF000000000000) {
      ...
      expired_time_high_ = std::numeric_limits<decltype(expired_time_high_)>::max();
      expired_time_low_ = std::numeric_limits<decltype(expired_time_low_)>::max();
      return;
    }
